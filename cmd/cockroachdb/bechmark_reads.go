...
func loadBenchDataset(db *sql.DB) *benchDataset {
	...
	// 3) groups: group_id,org_id -> org.member_group (in schema) via usergroup#member
	{
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb] loadBenchDataset: query groups: %v", err)
		}
		for rows.Next() {
		...
		`)
				if err != nil {
					...
					if err == sql.ErrNoRows {
						...
						log.Fatalf("[cockroachdb] loadBenchDataset: query group_hierarchy: %v", err)
					}
				} else {
					for rows.Next() {
						...
						if err := rows.Scan(&parent, &child, &relation); err != nil {
							rows.Close()
							log.Fatalf("[cockroachdb] loadBenchDataset: scan group_hierarchy: %v", err)
						}
						switch relation {
						case "member_group":
					...
					if err := rows.Err(); err != nil {
						log.Fatalf("[cockroachdb] loadBenchDataset: rows group_hierarchy: %v", err)
					}
					rows.Close()
				}
			}
			...
			if err := rows.Scan(&groupID, &orgID); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb] loadBenchDataset: scan groups: %v", err)
			}
			groupOrg[groupID] = orgID
		}
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb] loadBenchDataset: rows groups: %v", err)
		}
		rows.Close()
	}
