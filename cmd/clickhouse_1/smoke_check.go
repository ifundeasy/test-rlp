...
func RunClickhouseSmokeCheck() {
	...
	if err != nil {
		log.Fatalf("[clickhouse_1] smoke_check: connect failed: %v", err)
	}
	defer cleanup()
	...
	if err := db.QueryRowContext(ctx, `SELECT count() FROM user_resource_permissions`).Scan(&totalFlat); err != nil {
		log.Fatalf("[clickhouse_1] smoke_check: count user_resource_permissions failed: %v", err)
	}
	log.Printf("[clickhouse_1] smoke_check: user_resource_permissions rows=%d", totalFlat)

	if totalFlat == 0 {
		log.Printf("[clickhouse_1] smoke_check: warning - flattened table empty; maybe MV not created or data not loaded")
		return
	}
	...
	if err := db.QueryRowContext(ctx, `SELECT user_id FROM user_resource_permissions LIMIT 1`).Scan(&sampleUser); err != nil {
		log.Fatalf("[clickhouse_1] smoke_check: select sample user failed: %v", err)
	}
	...
	if err := db.QueryRowContext(ctx, `SELECT countDistinct(resource_id) FROM user_resource_permissions WHERE user_id = ?`, sampleUser).Scan(&countFlat); err != nil {
		log.Fatalf("[clickhouse_1] smoke_check: countDistinct flat failed: %v", err)
	}
	...
	if err := db.QueryRowContext(ctx, joinQuery, sampleUser).Scan(&countJoin); err != nil {
		log.Fatalf("[clickhouse_1] smoke_check: join-based count failed: %v", err)
	}

	log.Printf("[clickhouse_1] smoke_check: sampleUser=%d flat=%d join=%d", sampleUser, countFlat, countJoin)

	if countFlat != countJoin {
		log.Printf("[clickhouse_1] smoke_check: mismatch detected (flat vs join). This may indicate MV was not populated correctly.")
	} else {
		log.Printf("[clickhouse_1] smoke_check: OK - sample counts match")
	}
