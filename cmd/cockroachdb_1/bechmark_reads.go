...
func CockroachdbBenchmarkReads() {
	...
	// NOTE: Cockroachdb is PostgreSQL wire-compatible, we reuse the same helper.
	db, cleanup, err := infrastructure.NewCockroachFromEnv(ctx)
	if err != nil {
		log.Fatalf("[cockroachdb_1] benchmark: connect failed: %v", err)
	}
	defer cleanup()

	log.Println("[cockroachdb_1] == Building benchmark dataset from Cockroachdb ==")
	data := loadBenchDataset(db)
	log.Println("[cockroachdb_1] == Running Cockroachdb read benchmarks on DB-backed dataset ==")
	...
	log.Println("[cockroachdb_1] == Cockroachdb read benchmarks DONE ==")
}
...
func loadBenchDataset(db *sql.DB) *benchDataset {
	...
	// 1) org_memberships: org_id,user_id,role
	{
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: query org_memberships: %v", err)
		}
		for rows.Next() {
			var orgID, userID, role string
			if err := rows.Scan(&orgID, &userID, &role); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb_1] loadBenchDataset: scan org_memberships: %v", err)
			}
		...
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: rows org_memberships: %v", err)
		}
		rows.Close()
	}
	...
	// 2) group_memberships: group_id,user_id,role
	{
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: query group_memberships: %v", err)
		}
		for rows.Next() {
			var groupID, userID, role string
			if err := rows.Scan(&groupID, &userID, &role); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb_1] loadBenchDataset: scan group_memberships: %v", err)
			}
			_ = role // member/admin -> both counted via group membership
			groupMembers[groupID] = append(groupMembers[groupID], userID)
		}
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: rows group_memberships: %v", err)
		}
		rows.Close()
	}

	// 3) groups: group_id,org_id
	{
		rows, err := db.QueryContext(ctx, `
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: query groups: %v", err)
		}
		for rows.Next() {
			var groupID, orgID string
			if err := rows.Scan(&groupID, &orgID); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb_1] loadBenchDataset: scan groups: %v", err)
			}
			groupOrg[groupID] = orgID
		}
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: rows groups: %v", err)
		}
		rows.Close()
	}
	...
	// 4) resources: resource_id,org_id
	{
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: query resources: %v", err)
		}
		for rows.Next() {
			var resID, orgID string
			if err := rows.Scan(&resID, &orgID); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb_1] loadBenchDataset: scan resources: %v", err)
			}
			resOrg[resID] = orgID
		...
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: rows resources: %v", err)
		}
		rows.Close()
	}
...

	{
		...
		`)
		if err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: query resource_acl: %v", err)
		}
		for rows.Next() {
			var resID, subjectType, subjectID, relation string
			if err := rows.Scan(&resID, &subjectType, &subjectID, &relation); err != nil {
				rows.Close()
				log.Fatalf("[cockroachdb_1] loadBenchDataset: scan resource_acl: %v", err)
			}
		...
		if err := rows.Err(); err != nil {
			log.Fatalf("[cockroachdb_1] loadBenchDataset: rows resource_acl: %v", err)
		}
		rows.Close()
	}
...
func runCheckManageDirectUser(db *sql.DB, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[cockroachdb_1] [check_manage_direct_user] skipped: no direct manager ACL entries")
		return
	}
	...
	log.Printf("[cockroachdb_1] [%s] iterations=%d samplePairs=%d", name, iters, len(pairs))
	...
	log.Printf("[cockroachdb_1] [%s] DONE: iters=%d allowed=%d avg=%s total=%s",
...
func runCheckManageOrgAdmin(db *sql.DB, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[cockroachdb_1] [check_manage_org_admin] skipped: no org admin + resource pairs")
		return
	}
...
func runCheckViewViaGroupMember(db *sql.DB, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[cockroachdb_1] [check_view_via_group_member] skipped: no group-based viewer pairs")
		return
	}
...
func runLookupResourcesManageHeavyUser(db *sql.DB, data *benchDataset) {
	...
	if userID == "" {
		log.Printf("[cockroachdb_1] [lookup_resources_manage_super] skipped: no heavyManageUser found")
		return
	}

	name := "lookup_resources_manage_super"
	log.Printf("[cockroachdb_1] [%s] iterations=%d user=%s", name, iters, userID)
	...
	for i := 0; i < iters; i++ {
		...
		log.Printf("[cockroachdb_1] [%s] iter=%d resources=%d duration=%s",
			name, i, count, dur.Truncate(time.Millisecond))
	}

	avg := time.Duration(int64(total) / int64(iters))
	log.Printf("[cockroachdb_1] [%s] DONE: iters=%d lastCount=%d avg=%s total=%s",
...
func runLookupResourcesViewRegularUser(db *sql.DB, data *benchDataset) {
	...
	if userID == "" {
		log.Printf("[cockroachdb_1] [lookup_resources_view_regular] skipped: no regularViewUser found")
		return
	}
