...
func AuthzedDropSchemas() {
	...
	log.Printf("[authzed_pgdb_1] == Dropping relationships for known resource types ==")
	...
	for _, rt := range resourceTypes {
		if !schemaHasDefinition(schemaText, rt) {
			log.Printf("[authzed_pgdb_1] SKIP: no `definition %s` in current schema, nothing to delete", rt)
			continue
		}
		dropRelationshipsForType(client, rt)
	}

	log.Println("[authzed_pgdb_1] DONE: delete attempt finished for resource, organization, usergroup (only existing types were processed)")
}
...
func readCurrentSchema(client *authzed.Client) string {
	...
	if err != nil {
		log.Fatalf("[authzed_pgdb_1] ReadSchema failed: %v", err)
	}
...
func dropRelationshipsForType(client *authzed.Client, resourceType string) {
	log.Printf("[authzed_pgdb_1] Deleting all relationships with resource_type=%q ...", resourceType)
	...
	if err != nil {
		...
		log.Printf("[authzed_pgdb_1] DeleteRelationships for %s failed: %v", resourceType, err)
		return
	}

	log.Printf("[authzed_pgdb_1] Deleted all relationships for resource_type=%q", resourceType)
}
