...
func AuthzedCreateData() {
	...
	log.Printf("[authzed_pgdb_1] == Starting Authzed data import from CSV in %q ==", dataDir)
	...
	log.Printf("[authzed_pgdb_1] Authzed data import DONE: totalRelationships=%d elapsed=%s", relCount, elapsed)
}
...
func openCSV(name string) (*csv.Reader, *os.File) {
	...
	f, err := os.Open(full)
	if err != nil {
		log.Fatalf("[authzed_pgdb_1] open %s: %v", full, err)
	}
	r := csv.NewReader(f)
	return r, f
}
...
func loadOrgMemberships(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	if _, err := r.Read(); err != nil {
		log.Fatalf("[authzed_pgdb_1] read org_memberships header: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] read org_memberships row: %v", err)
		}
		if len(rec) < 3 {
			log.Fatalf("[authzed_pgdb_1] invalid org_memberships row: %#v", rec)
		}
	...
	log.Printf("[authzed_pgdb_1] Loaded org_memberships: %d relationships (cumulative=%d)", count, *relCount)
}
...
func loadGroups(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	if _, err := r.Read(); err != nil {
		log.Fatalf("[authzed_pgdb_1] read groups header: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] read groups row: %v", err)
		}
		if len(rec) < 2 {
			log.Fatalf("[authzed_pgdb_1] invalid groups row: %#v", rec)
		}
	...
	log.Printf("[authzed_pgdb_1] Loaded groups -> org.member_group: %d relationships (cumulative=%d)", count, *relCount)
}
...
func loadGroupMemberships(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	if _, err := r.Read(); err != nil {
		log.Fatalf("[authzed_pgdb_1] read group_memberships header: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] read group_memberships row: %v", err)
		}
		if len(rec) < 3 {
			log.Fatalf("[authzed_pgdb_1] invalid group_memberships row: %#v", rec)
		}
	...
	log.Printf("[authzed_pgdb_1] Loaded group_memberships: %d relationships (cumulative=%d)", count, *relCount)
}
...
func loadResources(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	if _, err := r.Read(); err != nil {
		log.Fatalf("[authzed_pgdb_1] read resources header: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] read resources row: %v", err)
		}
		if len(rec) < 2 {
			log.Fatalf("[authzed_pgdb_1] invalid resources row: %#v", rec)
		}
	...
	log.Printf("[authzed_pgdb_1] Loaded resources -> resource.org: %d relationships (cumulative=%d)", count, *relCount)
}
...
func loadResourceACL(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	if _, err := r.Read(); err != nil {
		log.Fatalf("[authzed_pgdb_1] read resource_acl header: %v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] read resource_acl row: %v", err)
		}
		if len(rec) < 4 {
			log.Fatalf("[authzed_pgdb_1] invalid resource_acl row: %#v", rec)
		}
		...
		case "user":
			...
			default:
				log.Fatalf("[authzed_pgdb_1] unknown ACL relation for user: %q", aclRelation)
			}
		...
		case "group":
			...
			default:
				log.Fatalf("[authzed_pgdb_1] unknown ACL relation for group: %q", aclRelation)
			}

		default:
			log.Fatalf("[authzed_pgdb_1] unknown subject_type in resource_acl: %q", subjectType)
		}
	...
	log.Printf("[authzed_pgdb_1] Loaded resource_acl: %d relationships (cumulative=%d)", count, *relCount)
}
...
func flushIfNeeded(client *authzed.Client, batch *[]*v1.RelationshipUpdate, relCount *int, start time.Time) {
	...
	*batch = (*batch)[:0]
	...
	if (*relCount)%1_000_000 == 0 {
		log.Printf("[authzed_pgdb_1] ... %d relationships inserted (elapsed=%s)",
...
func writeBatch(client *authzed.Client, batch []*v1.RelationshipUpdate) {
	...
	if err != nil {
		log.Fatalf("[authzed_pgdb_1] WriteRelationships failed: %v", err)
	}
}
