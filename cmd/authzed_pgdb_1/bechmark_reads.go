...
func AuthzedBenchmarkReads() {
	...
	if err != nil {
		log.Fatalf("[authzed_pgdb_1] failed to create authzed client: %v", err)
	}
	defer cancel()
	defer client.Close()

	log.Println("[authzed_pgdb_1] == Building benchmark dataset from SpiceDB ==")
	data := loadBenchDataset(client)
	log.Println("[authzed_pgdb_1] == Running Authzed read benchmarks on SpiceDB dataset ==")
	...
	log.Println("[authzed_pgdb_1] == Authzed read benchmarks DONE ==")
}
...
func loadBenchDataset(client *authzed.Client) *benchDataset {
	...
	readRels := func(filter *v1.RelationshipFilter, handle func(rel *v1.Relationship)) {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] ReadRelationships failed: %v", err)
		}
		for {
			...
			if err != nil {
				log.Fatalf("[authzed_pgdb_1] ReadRelationships stream recv failed: %v", err)
			}
...
func runCheckManageDirectUser(client *authzed.Client, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[authzed_pgdb_1] [check_manage_direct_user] skipped: no direct manager_user ACL entries")
		return
	}
	...
	log.Printf("[authzed_pgdb_1] [%s] iterations=%d samplePairs=%d", name, iters, len(pairs))
	...
	for i := 0; i < iters; i++ {
		...
		cancel()
		if err != nil {
			log.Fatalf("[authzed_pgdb_1] [%s] CheckPermission failed: %v", name, err)
		}
	...
	log.Printf("[authzed_pgdb_1] [%s] DONE: iters=%d allowed=%d avg=%s total=%s",
...
func runCheckManageOrgAdmin(client *authzed.Client, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[authzed_pgdb_1] [check_manage_org_admin] skipped: no org admin + resource pairs")
		return
	}
...
func runCheckViewViaGroupMember(client *authzed.Client, data *benchDataset) {
	...
	if len(pairs) == 0 {
		log.Printf("[authzed_pgdb_1] [check_view_via_group_member] skipped: no viewer_group-based sample pairs")
		return
	}
...
func runLookupResourcesManageHeavyUser(client *authzed.Client, data *benchDataset) {
	...
	if userID == "" {
		log.Printf("[authzed_pgdb_1] [lookup_resources_manage_super] skipped: no heavyManageUser found")
		return
	}

	name := "lookup_resources_manage_super"
	log.Printf("[authzed_pgdb_1] [%s] iterations=%d user=%s", name, iters, userID)
	...
	for i := 0; i < iters; i++ {
		...
		if err != nil {
			cancel()
			log.Fatalf("[authzed_pgdb_1] [%s] LookupResources failed: %v", name, err)
		}
		...
		for {
			...
			if err != nil {
				cancel()
				log.Fatalf("[authzed_pgdb_1] [%s] stream Recv failed: %v", name, err)
			}
		...
		log.Printf("[authzed_pgdb_1] [%s] iter=%d resources=%d duration=%s",
			name, i, count, dur.Truncate(time.Millisecond))
	}

	avg := time.Duration(int64(total) / int64(iters))
	log.Printf("[authzed_pgdb_1] [%s] DONE: iters=%d lastCount=%d avg=%s total=%s",
...
func runLookupResourcesViewRegularUser(client *authzed.Client, data *benchDataset) {
	...
	if userID == "" {
		log.Printf("[authzed_pgdb_1] [lookup_resources_view_regular] skipped: no regularViewUser found")
		return
	}
