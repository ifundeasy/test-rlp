...
func AuthzedBenchmarkReads() {
	...
	log.Println("[authzed_pgdb] == Building benchmark dataset from SpiceDB ==")
	data := loadBenchDataset(client)
	log.Println("[authzed_pgdb] == Running Authzed read benchmarks on SpiceDB dataset ==")
...
func loadBenchDataset(client *authzed.Client) *benchDataset {
	...
	readRels := func(filter *v1.RelationshipFilter, handle func(rel *v1.Relationship)) {
		...
		if err != nil {
			log.Fatalf("[authzed_pgdb] ReadRelationships failed: %v", err)
		}
		for {
			...
			if err != nil {
				log.Fatalf("[authzed_pgdb] ReadRelationships stream recv failed: %v", err)
			}
	...
	log.Printf("[authzed_pgdb] Benchmark dataset loaded in %s: directManagerPairs=%d orgAdminPairs=%d groupViewPairs=%d heavyManageUser=%q regularViewUser=%q",
...
func runCheckBench(client *authzed.Client, name, permission string, pairs []benchPair, iters int, timeout time.Duration) {
	if len(pairs) == 0 {
		log.Printf("[authzed_pgdb] [%s] skipped: no sample pairs", name)
		return
	}

	log.Printf("[authzed_pgdb] [%s] iterations=%d samplePairs=%d", name, iters, len(pairs))
	...
	for i := 0; i < iters; i++ {
		...
		cancel()
		if err != nil {
			log.Fatalf("[authzed_pgdb] [%s] CheckPermission failed: %v", name, err)
		}
	...
	log.Printf("[authzed_pgdb] [%s] DONE: iters=%d allowed=%d avg=%s total=%s",
